...
*/
    ...
    this.timeAndPersonResolution = function(qry, callback) {
        ...
        console.log(qry.intent + " " + qry.entity[0]);
        
        var readTxResultPromise = session.readTransaction(function (transaction) {
            // used transaction will be committed automatically, no need for explicit commit/rollback
            if (qry.entity.length > 1)
                var result = transaction.run('MATCH (q:Movies)-[]-(p:Persons) where q.releaseYear="' + qry.entity[0] + '" AND p.name="'+ qry.entity[1] + '" AND NOT q.genre contains "Adult" AND NOT q.genre contains "Talk-Show" AND NOT q.name contains "Episode" AND coalesce(q.rating,-1)>6 AND coalesce(q.rating, -1) < 9 RETURN q.name as name,q.genre as genre limit 25');
            else
